...
public class Graph {
	  ...
	  public static class Vertex implements Comparable<Vertex>{
		...
		private void printPath()
		{
			...
			{
				System.out.printf("%s(station name error)", this.name);
			}
			else
			{
	   ...
	   public Graph(Edge[] edges) {
	      ...
	      //another pass to set neighbour vertices
	      for (Edge e : edges) {
	         graph.get(e.v1).neighbours.put(graph.get(e.v2), e.dist);
	         graph.get(e.v2).neighbours.put(graph.get(e.v1), e.dist); // undirected graph
	      }
	   }
